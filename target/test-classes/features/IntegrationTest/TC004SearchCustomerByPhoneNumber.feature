...
Feature: Dealer via One Portal - Search Customer - by Mobile Phone Number
  ...
  Scenario Outline: Search customer by mobile phone number
    Given JIT: Tester calls the Account Get api for search phone number at "<basePath>"
    When JIT: Tester sends the get request with Headers phoneNumber ContentType "<ContentType>" XBRAND "<XBRAND>" XCHANNEL "<XCHANNEL>" XCORRELATIONID "<XCORRELATIONID>" Authorization "<Authorization>"
    Then JIT: The customer account info is retrieved by searching phoneNumber with StatusCode "<statusCode>" ResponseCode "<responseCode>" and Description "<description>"
