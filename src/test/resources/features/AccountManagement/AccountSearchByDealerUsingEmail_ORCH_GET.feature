@AcmDetails_Regression @AcSearchByDealerUsingEmail
Feature: Testing Account Search by Dealer using Email - Orchestration Micro Services
  As a tester, I should be able to search for a customer by Email as a Dealer(US_DEALER,CA_DEALER) by using Account Search by Dealer Micro service exposed from the Orchestration layer.
  Searches for a customer based on Email and returns minimum required customer information.

  @acm_smoke @AcmDetails_smoke @AcSearchByDealerUsingEmail-Smoke
  Scenario Outline: Dealer Search for a Customer's Account by Email using Dealer Search API exposed from Orchestration layer using valid input parameters (headers) and expect status code 200 OK <scenarioName>
  ...
  @AcSearchByDealerUsingEmail-Positive
  Scenario Outline: Dealer Search for a Customer's Account by Email using Dealer Search API exposed from Orchestration layer using valid input parameters (headers) and expect status code 200 OK <scenarioName>
    Given Tester calls the Create Customer Account POST api at "<basePath>"
    And Tester uses the headers with ContentType "<ContentType>" XBRAND "<XBRAND>" XCHANNEL "<XCHANNEL>" XCORRELATIONID "<XCORRELATIONID>" Authorization "<Authorization>"
    When Tester post the service with json body having customerType "<customerType>" firstName "<firstName>" lastName "<lastName>" email "<email>" phone "<phone>"
  ...
  @AcSearchByDealerUsingEmail-MissingHeaders
  Scenario Outline: Dealer Search for a Customer's Account by Email using Dealer Search API exposed from Orchestration layer with missing Headers and expect status code 400 <scenarioName>
    Given The Dealer Search for a Customer Account GET api service "<basePath>" is up
...
#Removing both Key & Value pairs
      ...
      | with T -  US_DEALER Channel     | /v1/account | application/json | MissingHeader | US_DEALER     | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      | with T -  US_DEALER Channel     | /v1/account | application/json | T             | MissingHeader | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      | with T -  US_DEALER Channel     | /v1/account | application/json | T             | US_DEALER     | MissingHeader   | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      ...
      | with T -  US_DEALER Channel     | /v1/account | application/json | T             | US_DEALER     | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | MissingHeader | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |

      | with L -  US_DEALER Channel     | /v1/account | application/json | L             | MissingHeader | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      | with L -  US_DEALER Channel     | /v1/account | application/json | L             | US_DEALER     | MissingHeader   | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      ...
      | with L -  US_DEALER Channel     | /v1/account | application/json | L             | US_DEALER     | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | MissingHeader | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |

#Passing Empty values

      | with T -  US_DEALER Channel     | /v1/account | application/json |               | US_DEALER     | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      | with T -  US_DEALER Channel     | /v1/account | application/json | T             |               | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      | with T -  US_DEALER Channel     | /v1/account | application/json | T             | US_DEALER     |                 | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      ...
      | with T -  US_DEALER Channel     | /v1/account | application/json | T             | US_DEALER     | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t |               | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |

      | with L -  US_DEALER Channel     | /v1/account | application/json | L             |               | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      | with L -  US_DEALER Channel     | /v1/account | application/json | L             | US_DEALER     |                 | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | test@test.com | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |
      ...
      | with L -  US_DEALER Channel     | /v1/account | application/json | L             | US_DEALER     | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t |               | 400        | ORCH-0001    | Mandatory Header Attribute(s) Missing |


  @AcSearchByDealerUsingEmail-InvalidHeaders
  Scenario Outline: Dealer Search for a Customer's Account by Email using Dealer Search API exposed from Orchestration layer using invalid headers and expect status code 400 <scenarioName>
    Given The Dealer Search for a Customer Account GET api service "<basePath>" is up
    When Tester uses the headers with ContentType "<ContentType>" XBRAND "<XBRAND>" XCHANNEL "<XCHANNEL>" XCORRELATIONID "<XCORRELATIONID>" Authorization "<Authorization>" email "<email>"
    And Tester calls the GET service
    Then the Customer account details should be searched in OCPR system by returning Status code "<statusCode>"
...
#      | with T -  Domian name without period                  | /v1/account | application/json | T      | TMNA_AGENT | sendThroughCode                       | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | Toyotaca_self_20180709_14@tdomaincom             | 400        | ORCH-0002    | Incorrect/Invalid Header Values |
...
#      | with L -  Domian name without period                  | /v1/account | application/json | L      | TMNA_AGENT | sendThroughCode                       | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | Toyotaca_self_20180709_14@tdomaincom             | 400        | ORCH-0002    | Incorrect/Invalid Header Values |
  ...
  @AcSearchByDealerUsingEmail-InvalidUser
  Scenario Outline: Dealer Search for a Customer's Account by Email using Dealer Search API exposed from Orchestration layer using invalid user email and expect status code 404 <scenarioName>
    Given The Dealer Search for a Customer Account GET api service "<basePath>" is up
    When Tester uses the headers with ContentType "<ContentType>" XBRAND "<XBRAND>" XCHANNEL "<XCHANNEL>" XCORRELATIONID "<XCORRELATIONID>" Authorization "<Authorization>" email "<email>"
    And Tester calls the GET service
    Then the Customer account details should be searched in OCPR system by returning Status code "<statusCode>"
    ...
    Examples:
      | scenarioName                      | basePath    | ContentType      | XBRAND | XCHANNEL  | XCORRELATIONID  | Authorization                            | email                               | statusCode | responseCode | description    |
      | with email wihtout having account | /v1/account | application/json | T      | US_DEALER | sendThroughCode | icZBMDlw2XOtuTLIZkfeFi1vTE8QXr4lr8tA6b7t | testEmail-notRegistered@tetsing.com | 404        | OCPR-0011    | User Not Found |

#  @AcSearchByDealerUsingEmail-InternalServerError/SystemNotAvailable
#  Scenario Outline: Dealer Search for a Customer's Account by Email using Dealer Search API exposed from Orchestration layer using valid input when InternalServerError/SystemNotAvailable and expect status code 500 <scenarioName>
