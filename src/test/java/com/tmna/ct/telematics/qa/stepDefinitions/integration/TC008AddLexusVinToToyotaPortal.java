...
public class TC008AddLexusVinToToyotaPortal extends TestBase{
    /**
     * @author Henry Peng
     * 
     * Feature file: /resources/features/IntegrationTest/TC008AddLexusVinToToyotaPortal.feature
     * 
     * Steps:
     * 1. Get the lexus vin that already generated from Data setup
     * 2. Create a new customer account at toyota portal
     * 3. Add vehicle association
     * 4. Create Subscription
     * 5. Find vehicle, verify vehicle Status should be SUBSCRIBED.
     * 
     * Feature file tags @AddLexusVinToToyotaPortal pair with @DataSetUp in IntegrationTestRunner.
     */

    //Step 1: Use the lexus vin that generated from data set up
	
    //Step 2: Create Account is a common scenario, which is located in CommonStepDefs.
	
	//Step 3: Search lexus vin that generate from data setup
   @Given("^JIT: Tester calls the GET request with Brand lexus \"([^\"]*)\" vin at \"(.*?)\"$")
    public void tester_calls_the_get_api_with_basePath_lexus_vin(String brand, String path) throws Throwable {
    	baseURI = configProps.getProperty("CVS_PREFIX") + APILibrary.getEnvironmentName() + configProps.getProperty("BASEURI_SUFFIX_CVS");

    	request = APILibrary.getConfigAPI(path + Datasetup.lexusVinList.get(0));
    }
    
   @And("^JIT: Tester set the headers for adding Lexus vin with ContentType \"([^\"]*)\" XCHANNEL \"([^\"]*)\" XCORRELATIONID \"([^\"]*)\" xapikey \"([^\"]*)\"$")
   ...
   @When("^JIT: Tester sends the GET request for lexus vin$")
    public void tester_calls_the_GET_service_for_enter_valid_lexus_vin() throws Throwable {
    	response = APILibrary.callGETRequest();
        result = APILibrary.getJsonPathResult();
    }
    
   @Then("^JIT: The lexus vehicle info is retrieved by returning Status code \"([^\"]*)\"$")
    public void the_invalid_lexus_vin_code_can_get_result_by_returning_status_code(int expectedStatusCode) throws Throwable {
    	validation.verifyStatusCode(response, expectedStatusCode);
    }
    
    @And("^JIT: The server returns the lexus vin with ResponseCode \"([^\"]*)\" Description \"([^\"]*)\"$")
    ...
    @And("^JIT: Tester set the headers for Lexus packages with ContentType \"([^\"]*)\" XBRAND \"([^\"]*)\" XCHANNEL \"([^\"]*)\" XCORRELATIONID \"([^\"]*)\" Authorization \"([^\"]*)\" DATETIME \"([^\"]*)\" asicode \"([^\"]*)\" hwtype \"([^\"]*)\" generation \"([^\"]*)\" region \"([^\"]*)\"$")
    ...
    @Then("^JIT: The lexus products GET api response returns \"([^\"]*)\" status code$")
    ...
    @And("^JIT: Tester set Create lexus vehicle-association headers with Guid ContentType \"([^\"]*)\" XBRAND \"([^\"]*)\" XCHANNEL \"([^\"]*)\" XCORRELATIONID \"([^\"]*)\" Authorization \"([^\"]*)\" DATETIME \"([^\"]*)\"$")
    ...
    @When("^JIT: Tester post lexus Vehicle Association POST api service with payload resource generation \"([^\"]*)\" region \"([^\"]*)\" preferredDealerCode \"([^\"]*)\" role \"([^\"]*)\" nickName \"([^\"]*)\" status \"([^\"]*)\" createDate \"([^\"]*)\" createSource \"([^\"]*)\"$")
    public void tester_post_ORCH_Orchestration_Vehicle_Association_POST_api_service_with_json_body_having_vin_generation_region_preferredDealerCode_role_nickName_status_createDate_createSource(String generation, String region, String preferredDealerCode, String role, String nickName, String status, String createDate, String createSource) throws Throwable {
    	...
    	Vehicle vehicle = new Vehicle(Datasetup.lexusVinList.get(0), generation, region, preferredDealerCode, role, nickName, status, createDate, createSource);
    ...
    @Then("^JIT: Lexus Vehicle Association create POST api server returns \"([^\"]*)\" status code$")
    ...
    @And("^JIT: Lexus Vehicle Association create POST api body response content should have the ResponseCode \"([^\"]*)\" and Description \"([^\"]*)\"$")
    ...
    @And("^JIT: Tester set the headers to subscribe lexus vehicle with ContentType \"([^\"]*)\" XBRAND \"([^\"]*)\" XCHANNEL \"([^\"]*)\" XCORRELATIONID \"([^\"]*)\" Authorization \"([^\"]*)\" DATETIME \"([^\"]*)\"$")
    ...
    @When("^JIT: Tester post the service with payload resource having lexus vin subscriberGuid region \"([^\"]*)\" generation \"([^\"]*)\" waiver \"([^\"]*)\" remoteUserGuid \"([^\"]*)\" packageID productID type ratePlanID term termUnit subscriptionStartDate can \"([^\"]*)\" serviceConnect \"([^\"]*)\" dealerContact \"([^\"]*)\" ubi \"([^\"]*)\" createDate \"([^\"]*)\" createSource \"([^\"]*)\" termsAcceptanceDate \"([^\"]*)\"$")
    public void tester_post_the_service_with_payload_having_vin_subscriberGuid_region_generation_waiver_remoteUserGuid_packageID_productID_type_ratePlanID_term_termUnit_subscriptionStartDate_can_serviceConnect_dealerContact_ubi_createDate_createSource_termsAcceptanceDate(String region, String generation, Boolean waiver, String remoteUserGuid, String can, String serviceConnect, String dealerContact, String ubi, String createDate, String createSource, String termsAcceptanceDate ) throws Throwable {
        ...
        OrchSubCreate jsonBodyObject = OrchSubCreate.builder()
        		.vin(Datasetup.lexusVinList.get(0))
    ...
    @Then("^JIT: Lexus subscription POST API returns the response with \"([^\"]*)\" status code$")
    public void the_server_returns_the_orch_subscription_status_code(int expectedStatusCode) {
    	
    	validation.verifyStatusCode(response, expectedStatusCode);
    }

    @And("^JIT: Lexus subscription POST API body response should have the ResponseCode \"([^\"]*)\" and Description \"([^\"]*)\"$")
    ...
    public void tester_calls_the_ORCH_subscription_get_request_at() {
        baseURI = configProps.getProperty("ORCH_SUB_PREFIX") + APILibrary.getEnvironmentName() + configProps.getProperty("BASEURI_SUB_SUFFIX");
        request = APILibrary.getConfigAPI(configProps.getProperty("OrchGetDetailsSubscription") + Datasetup.lexusVinList.get(0));
    }
    
    @And("^JIT: Tester sents subscription GET request for lexus vin having headers with ContentType \"([^\"]*)\" XBRAND \"([^\"]*)\" XCHANNEL \"([^\"]*)\" XCORRELATIONID \"([^\"]*)\" Authorization \"([^\"]*)\" DATETIME \"([^\"]*)\"$")
    ...
    @Then("^JIT: Tester verify lexus vehicle status should be \"([^\"]*)\" status$")
