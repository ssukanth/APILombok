...
public class TC006CreateAccountForExistingCustomerWithPhoneNumber extends TestBase{
	/**
     * @author Henry Peng
     * 
     * Feature file: /resources/features/IntegrationTest/TC006CreateAccountForExistingCustomerWithPhoneNumber.feature
     * 
     * Steps:
     * 1. Create an account
     * 2. Continue creating an account for an already existing customer with phone number
     * 
     */

    //Step 1: Create Account is a common scenario, which is located in CommonStepDefs.
	
	//Step 2: Create an account for an already existing customer with phone number
	@Given("^JIT: To create an account for an already existing Customer with Phone Number, tester calls the Create Customer Account POST api at \"([^\"]*)\"$")
    ...
    @Given("^JIT: To create an account, tester uses the headers with ContentType \"([^\"]*)\" XBRAND \"([^\"]*)\" XCHANNEL \"([^\"]*)\" XCORRELATIONID \"([^\"]*)\" Authorization \"([^\"]*)\"$")
    ...
    @When("^JIT: To create an account, tester post the service with json body having customerType \"([^\"]*)\" firstName \"([^\"]*)\" lastName \"([^\"]*)\" email \"([^\"]*)\" phone \"([^\"]*)\"$")
    public void tester_post_the_service_with_json_body_having_guid_objectId_customerType_firstName_lastName_email_phone_password(String customerType, String firstName, String lastName, String email, String phone) throws Throwable {
    	 CreateCustomerAcount_ORCH_Pojo jsonBodyObject = CreateCustomerAcount_ORCH_Pojo.builder()
         		...
         		.firstName(firstName)
         		.lastName(lastName)
         		.email(APILibrary.generateEmail(email))
         		.phoneNumber(CommonStepDefs.phoneNumber)
         		.build();
    ...
    @Then("^JIT: Can not create an account for an already existing Customer with mobile phone number with StatusCode \"([^\"]*)\" ResponseCode \"([^\"]*)\" and Description \"([^\"]*)\"$")
